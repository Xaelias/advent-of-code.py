...
class PartOne(PartZero):
    def parse_line(self, line: str) -> tuple[range, range]:
        return tuple(seq(re.findall(r"\d+", line)).map(int).grouped(2).starmap(range))
    ...
    def process(self) -> int:
        return (
            ...
            .map(self.parse_line)
            .count(lambda t: (
                t[0].start <= t[1].start and t[0].stop >= t[1].stop or
                t[1].start <= t[0].start and t[1].stop >= t[0].stop
            ))
        )
