...
TEST_INPUT_RESULT = 24000
REAL_INPUT_RESULT = 69883
...
class PartZero(ABC):
    ...
    def parse(self, convert: Callable) -> list["???"]:
        return self.as_seq.map(convert).to_list()  # type: ignore

class PartOne(PartZero):
    def parse(self) -> list[list[int]]:
        l = [[]]
        for line in self.as_str:
            line = line.strip()
            if line == "":
                l.append([])
                continue
