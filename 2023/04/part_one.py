...
def process_line(line: str) -> int:
    ...
    logger.info(f"Processed line. [Card {card}: {numbers=}, {winning=}, {matches=}, {points=}]")
    return points


def process_input(iterable) -> int:
    return sum((process_line(line) for line in iterable))
...
def test_process_line():
    assert process_line(TEST_INPUT[0]) == 8
